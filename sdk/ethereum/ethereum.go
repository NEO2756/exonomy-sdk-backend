...
func (e *Ethereum) GetTokenBalance(tokenAddress, address string) decimal.Decimal {
	...
	idStr := e.GetAccountID(address)
	accountID, _ := strconv.Atoi(idStr)
	
	res, err := e.client.EthCall(ethrpc.T{
		To: os.Getenv("HSK_HYBRID_EXCHANGE_ADDRESS"),
		From: address,
		Data: fmt.Sprintf("0x017b9d12000000000000000000000000%s%064x", without0xPrefix(tokenAddress), accountID),
	}, "latest")

	if err != nil {
		panic(err)
	}
    //fmt.Println("balance", utils.StringToDecimal(res), tokenAddress);
	return utils.StringToDecimal(res)
}

func (e *Ethereum) GetAccountID (address string) string {
	
	res, err := e.client.EthCall(ethrpc.T{
		To: os.Getenv("HSK_HYBRID_EXCHANGE_ADDRESS"),
		From: address,
		Data: fmt.Sprintf("0xcc74d6d8000000000000000000000000%s", without0xPrefix(address)),
	}, "latest")
    ...
    fmt.Println("accountID", utils.StringToDecimal(res));
	return res
}
